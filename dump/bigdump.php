...
// 数据库连接配置（本地XAMPP专用）
$db_server   = 'localhost'; // 本地服务器，固定填localhost即可
$db_name     = 'restaurant'; // 替换成你在phpMyAdmin创建的数据库名（必填）
$db_username = 'root'; // XAMPP默认MySQL用户名，若改过则填修改后的
$db_password = ''; // 无密码！直接填空字符串，单引号中间无任何字符
// SQL文件编码（本地建议用UTF-8，和你的SQL文件一致）
...
$buffer_size = 65536; // 同时增大缓冲区（64KB），提升语句解析能力
$lines_per_session = 500; // 保持较小的单次执行行数，避免超时
